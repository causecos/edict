...
def main():
    # 使用 officials_stats.json（與 sync_officials_stats.py 統一）
    ...
    # 任務源優先：tasks_source.json（可對接外部系統同步寫入）
    ...
    for t in tasks:
        ...
        # 心跳時效檢測：對 Doing/Assigned 狀態的任務標註活躍度
        if t.get('state') in ('Doing', 'Assigned', 'Review'):
            ...
            if age_sec is None:
                ...
                t['heartbeat'] = {'status': 'active', 'label': f'🟢 活躍 {int(age_sec//60)}分鐘前', 'ageSec': int(age_sec)}
            elif age_sec < 900:
                t['heartbeat'] = {'status': 'warn', 'label': f'🟡 可能停滯 {int(age_sec//60)}分鐘前', 'ageSec': int(age_sec)}
            else:
                t['heartbeat'] = {'status': 'stalled', 'label': f'🔴 已停滯 {int(age_sec//60)}分鐘', 'ageSec': int(age_sec)}
        else:
            t['heartbeat'] = None
    ...
    for t in tasks:
        if t.get('state') == 'Done':
            ...
            history.append({
                ...
                'task': t.get('title'),
                'out': t.get('output'),
                'qa': '通過' if t.get('outputMeta', {}).get('exists') else '待補成果'
            })
