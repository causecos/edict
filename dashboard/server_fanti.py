...
def handle_scheduler_retry(task_id, reason=''):
    ...
    _scheduler_add_flow(task, f'觸發重試第{sched["retryCount"]}次：{reason or "超時未推進"}')
    task['updatedAt'] = now_iso()
    save_tasks(tasks)

    dispatch_for_state(task_id, task, state, trigger='taizi-retry')
    return {'ok': True, 'message': f'{task_id} 已觸發重試派發', 'retryCount': sched['retryCount']}
...
def handle_scheduler_rollback(task_id, reason=''):
    ...
    set_task_state(task_id, snap_state, f'↩️ 太子調度自動回滾：{reason or "恢復到上個穩定節點"}')
    record_task_flow(task_id, old_state, snap_state, f'執行回滾：{old_state} → {snap_state}，原因：{reason or "停滯恢復"}')
    ...
    task2['org'] = snapshot.get('org', task2.get('org', ''))
    task2['block'] = '無'
    sched2 = _ensure_scheduler(task2)
    sched2['retryCount'] = 0
    sched2['escalationLevel'] = 0
    sched2['stallSince'] = None
    sched2['lastProgressAt'] = now_iso()
    task2['updatedAt'] = now_iso()
    save_tasks(tasks2)

    if snap_state not in _TERMINAL_STATES:
        dispatch_for_state(task_id, task2, snap_state, trigger='taizi-rollback')
...
def handle_scheduler_scan(threshold_sec=600):
    ...
    for task in tasks:
        ...
        if retry_count < max_retry:
            ...
            _scheduler_add_flow(task, f'停滯{stalled_sec}秒，觸發自動重試第{sched["retryCount"]}次')
        ...
        if level < 2:
            ...
            sched['escalationLevel'] = next_level
            sched['lastEscalatedAt'] = now_iso()
            _scheduler_add_flow(task, f'停滯{stalled_sec}秒，升級至{target_label}協調', to=target_label)
        ...
        if sched.get('autoRollback', True):
            ...
            if rollback_count >= max_rollback:
                # 已達最大回滾次數，標記 Blocked 避免無限循環
                if state != 'Blocked':
                    task['state'] = 'Blocked'
                    task['now'] = f'🚫 連續回滾{rollback_count}次仍無法推進，已自動掛起'
                    task['block'] = f'連續停滯且回滾{rollback_count}次均失敗，需人工介入'
                    sched['stallSince'] = None
                    _scheduler_add_flow(task, f'連續回滾{rollback_count}次，自動掛起等待人工介入')
                ...
                sched['stallSince'] = None
                sched['lastProgressAt'] = now_iso()
                _scheduler_add_flow(task, f'連續停滯，自動回滾：{old_state} → {snap_state}（第{rollback_count + 1}次）')
